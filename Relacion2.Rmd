...
```{r}
regresion <- lm(Peso ~ Altura, data = datos2)
y_predict <- predict(regresion, datos2)

ggplot(data=datos2, aes(Peso,Altura)) + 
  geom_point()+
  geom_line(aes( x = datos2$Peso, y = y_predict), color = "red") +
  labs(
...
```{r}
regresion <- lm(Peso ~ Altura, data = datos2new)
y_predict <- predict(regresion, datos2new)
summary(regresion)
ggplot(data=datos2new, aes(Peso,Altura)) + 
  geom_point()+
  #geom_line(aes( x = datos2new$Peso, y = y_predict), color = "red") +
  labs(
    title = "Nube de puntos con recta de mínimos cuadrados.",
    subtitle = "Altura y peso",
    y="Altura (cm)",
